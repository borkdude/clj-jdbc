(ns clj-jdbc.impl.nrepl-server
  {:no-doc true}
  (:refer-clojure :exclude [send future binding])
  (:require [bencode.core :refer [read-bencode]]
            [clj-jdbc.impl.nrepl-server.utils :refer [dev? response-for send send-exception
...
(defn start-server! [ctx host+port]
  (vreset! dev? (= "true" (System/getenv "CLJ-JDBC_DEV")))
  (let [ctx (assoc ctx :sessions (atom #{}))
    ...
    (println "For more info visit https://github.com/borkdude/clj-jdbc/blob/master/doc/repl.md#nrepl.")
